...
mName <- "don1cpt.model.r" # the model file put in the model folder
inName <- "don1cpt.mc.in.r" # the input file put in the infile folder
...
colnames(out)<-c('iter', "km_d15g", "km_d3g", "ku_d15g", "ku_d3g", "ke_don", "t0",'t1','t2','t3','t4','t5','t6','t7','t8','t9','t10', 't11', 't12', 't13', 't14','t15', 't16', 't17', 't18','t19','t20','t21','t22','t23','t24')

vars <- names(out)
index <- which(vars == "t0" | vars == "t24")
...
y<-c(0, 0.9448, 8.0286, 6.9637, 5.5285, 8.4916, 3.7691, 4.2321, 3.6765, 1.2689, 0.9911, 1.0837, 0.6207, 0.4355, 0.8985, 1.6393, 1.6393, 3.2135, 0.7596, 1.6856, 0.7133, 0.7133, 0.2503, 0.4587, 0.2503)
...
# Define the input variable
mName <- "don1cpt.model.R" # the model file put in the model folder
inName <- "don1cptv5.mcmc.in.R" # the input file put in the infile folder
...
X_setpts <- mcsim("don1cpt.model.R", "don1cpt_setpts.in.R")
...
b<-read.csv("d15g_interval_obs.csv")
b<-as.data.frame(b)
...
##specifiy the column range and 2= "column [1=row]
don_a <- apply(don[index[1]:index[2]], 2, quantile,  c(0.05, 0.5, 0.95)) %>% t()
...
ggplot(don_a, aes(x = x, y = mean)) +
  ...
  geom_line() +
  geom_point(data = obs_data, x=x, y=y) +
  labs(x = "Time", y = "% excreted(cumulative)")
...
indi_pred<-read.csv('don_interval_pred.csv')
...
##specifiy the column range and 2= "column [1=row]
d3g_a <- apply(d3g[index[1]:index[2]], 2, quantile,  c(0.05, 0.5, 0.95)) %>% t()
...
indi_pred<-read.csv('d3g_interval_pred.csv')
indi_pred<-as.data.frame(indi_pred)
ggplot(indi_pred, aes(x = time, y = pred)) +
  geom_line() +
  geom_point(data=obs_data, x=x, y=z) +
